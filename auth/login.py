...
from auth.cookie_manager import check_cookie_exists, save_cookies
from auth.qrcode_login import QrCodeLoginThread, get_qr_code, show_qr_code_dialog


def handle_login_success(cookies, dialog, window):
    """处理登录成功后的操作"""
...
def on_scan_login_clicked(window):
    ...
    if url and qrcode_key:
        ...
        thread.login_success.connect(lambda cookies: handle_login_success(cookies, dialog, window))
        thread.start()  # 启动线程
...
def on_cookie_login_clicked(window):
    """通过cookie登录"""
    dialog = CookieLoginDialog(window)
    if dialog.exec_():  # 如果登录成功
        handle_login_success(dialog.cookies, dialog, window)
